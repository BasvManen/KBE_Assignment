...
class MomentOfInertia(GeomBase):
    ...
    def cutout_curves(self):
        equidistant_spacing = self.span/2/(self.n_cuts-1)
        cutout_list = []
    ...
    @Part(in_tree=False)
    def spanwise_cutout_curves(self):
    ...
    def coordinates_sections_points(self):
        crv = []
        for i in range(self.n_cuts):
            crv.append(self.spanwise_cutout_curves[i].equispaced_points(self.n_discretise))
        return crv
    ...
    def centroid_calculation(self):
        ...
        for i in range(self.n_cuts):
            ...
            z = 0
            for j in range(self.amount_of_lines):
                x = x + (self.lines_on_airfoil[i][j].midpoint.x * self.lines_on_airfoil[i][j].length * self.thickness) \
                    / (self.amount_of_lines * self.lines_on_airfoil[i][j].length * self.thickness)
                z = z + (self.lines_on_airfoil[i][j].midpoint.z * self.lines_on_airfoil[i][j].length * self.thickness) \
                    / (self.amount_of_lines * self.lines_on_airfoil[i][j].length * self.thickness)
    ...
    def moment_inertia_displaced_area(self):
        ...
        for i in range(self.n_cuts):
            ...
            ixz_area = 0
            for j in range(self.amount_of_lines):
                ix_area = ix_area + (self.lines_on_airfoil[i][j].length * self.thickness) \
                          * (self.lines_on_airfoil[i][j].midpoint.z - self.centroid_calculation[i][2]) ** 2
                iz_area = iz_area + (self.lines_on_airfoil[i][j].length * self.thickness) \
                          * (self.lines_on_airfoil[i][j].midpoint.x - self.centroid_calculation[i][0]) ** 2
                ixz_area = ixz_area + (self.lines_on_airfoil[i][j].length * self.thickness) \
    ...
    def moment_inertia_area_rectangle(self):
        ...
        for i in range(self.n_cuts):
            ...
            ixz = 0
            for j in range(self.amount_of_lines):
                iu = 1 / 12 * self.lines_on_airfoil[i][j].length * self.thickness ** 3
                iw = 1 / 12 * self.thickness * self.lines_on_airfoil[i][j].length ** 3
